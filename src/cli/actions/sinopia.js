...
module.exports = function (self) {
  const proxy = Boolean(process.env.http_proxy);
  const SPINNER_TTL = 2000;
  const cfgFile = path.join(process.env.RUNTIME_DIR, 'config.yaml');

  const isSinopiaInstalled = utilities.isAppInstalled({
    name: 'sinopia',
