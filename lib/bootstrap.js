    ...
    inquirer.prompt(questions, function (answers) {
      if (answers.proxy) {
        ...
        if (options.bootstrap) {
          msg.push('To take your proxy into account,\nyou need to restart your session before going any further...\nSee you soon!');
        } else {
          msg.push('To take your proxy into account, you need to restart your session.');
        }
        if (process.env.ENVTOOLS_VERSION) {
          msg.push('Hint: type r ENTER or just restart your terminal...');
        }
        done(-2, msg);
      } else {
        done(-1);
