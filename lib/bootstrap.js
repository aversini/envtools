...
function _bootstrapEnv(options, callback) {
  ...
  if (options.bootstrap && process.env.ENVTOOLS_VERSION) {
...
function _bootstrapProxy(options, callback) {
  ...
  if (options.bootstrap && fs.existsSync(ENVTOOLS.PROXY_FILE)) {
    return callback(null, options);
  }
  ...
  async.waterfall([
    ...
    if (options.bootstrap && err === USER_INTERRUPT) {
      err = null;
    }
...
function _bootstrapMaven(options, callback) {
  ...
  async.waterfall([
    ...
    if (err && (err === USER_INTERRUPT || err === -2)) {
      err = null;
    }
    callback(err, options);
  });
