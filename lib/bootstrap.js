...
function _bootstrapMaven(options, callback) {
  ...
  async.waterfall([
    ...
    function (done) {
      resultingFolder = path.join(RUNTIME_DIR, 'apache-maven-3.3.9');
...
function _bootstrapAtom(options, callback) {
  async.waterfall([
    ...
    function (done) {
      var
        cmdline = _.map(ATOM_PACKAGES, function (pkg) {
...
function _routeRequest(request, options, callback) {
  ...
  case REQUEST_PROMPT:
    ...
    break;
  case REQUEST_BANNER:
    _bootstrapBanner(options, callback);
    break;
  ...
  case REQUEST_FEDTOOLS:
    ...
    break;
  case REQUEST_QUICKLOOK:
    _installQuickLookPlugins(options, callback);
    break;
  case REQUEST_SCREENSAVER:
    _fixScreensaver(options, callback);
    break;
  case REQUEST_ATOM:
    _bootstrapAtom(options, callback);
    break;
...
function _displayOptions(type, options, callback) {
  var
    introMsg = [],
    totalChoices = 0,
  ...
  if (type === TYPE_MANUAL) {
    introMsg.push(options.i18n.t('bootstrap.intro.manual'));
    introMsg.push(options.i18n.t('bootstrap.intro.getOutAdvice2'));
  ...
  if (type === TYPE_EXTRA) {
    introMsg.push(options.i18n.t('bootstrap.intro.extra'));
    introMsg.push(options.i18n.t('bootstrap.intro.getOutAdvice3'));
  ...
  log.printMessagesInBox(introMsg);
  log.echo();
  inquirer.prompt(questions, function (answers) {
...
exports.manual = function (options, done) {
  ...
  _createRuntimeDir(function () {
    ...
    _displayOptions(TYPE_MANUAL, options, function (err) {
      if (err && (err === -1 || err === -2)) {
        err = null;
      }
      log.echo();
...
exports.extra = function (options, done) {
  ...
  _createRuntimeDir(function () {
    // and display a welcome message and options
    _displayOptions(TYPE_EXTRA, options, function (err) {
      if (err && (err === -1 || err === -2)) {
        err = null;
      }
      log.echo();
...
exports.auto = function (options, done) {
  ...
  _createRuntimeDir(function () {
    ...
    async.waterfall([
      ...
      function (callback) {
        _routeRequest(REQUEST_BANNER, options, function (err) {
          _trapStandardError(err, callback);
        });
      },
      function (callback) {
