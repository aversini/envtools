...
var
  ...
  // Maven 3.3.9 is not compatible with the framework for now...
  // Ticket has been opened: PN-13533
...
function _bootstrapEnv(options, callback) {
  ...
  function _writeDotProfile(file, buffer, done) {
    ...
    buffer.push('source ' + path.join(__dirname, '..', 'shell', 'load.sh'));
  ...
  inquirer.prompt(questions).then(function (answers) {
    if (answers.change) {
      ...
      return callback(USER_INTERRUPT, options);
    }
  });
}
...
function _bootstrapEslintConfiguration(options, callback) {
  ...
  async.waterfall([
    ...
    function (done) {
      var
        srcFile = path.join(ENVTOOLS.THIRDDIR, 'eslint', 'dot.eslintrc.json'),
        cmdline = 'cp -f ' + srcFile + ' ' + DOT_ESLINT_CFG;
      cmd.run(cmdline, {
        status: false
      }, function (err) {
        done(err);
  ...
  async.waterfall([
    ...
    function (done) {
      var
        ...
        cmdline = 'cp -f ' + srcFile + ' ' + DOT_JSBEAUTIFY_CFG;
      cmd.run(cmdline, {
        status: false
      }, function (err) {
        done(err);
...
function _bootstrapNodeAndNpm(options, callback) {
  ...
  async.waterfall([
    ...
    function (goForIt, done) {
      var cmdline;
      if (goForIt) {
        if (process.env.SINOPIA_STATUS && process.env.SINOPIA_STATUS === ON) {
...
function _bootstrapHomebrew(options, callback) {
  ...
  if (process.platform !== 'darwin') {
    ...
    return callback(USER_FATAL, options);
  } else {
    async.waterfall([
        function (done) {
          var
...
function _bootstrapSinopia(options, callback) {
  async.waterfall([
    function (done) {
      var
        questions = {
    ...
    function (toggleSinopia, done) {
      fs.writeFile(ENVTOOLS.SINOPIA_STATUS, toggleSinopia, function () {
    ...
    function (toggleSinopia, done) {
      ...
      if (toggleSinopia === ON) {
        ...
        cmd.run(cmdline, {
          ...
          if (fs.existsSync(ENVTOOLS.PROXY_FILE) &&
            fs.existsSync(ENVTOOLS.PROXY_STATUS_FILE)
          ) {
            proxyStatus = fs.readFileSync(ENVTOOLS.PROXY_STATUS_FILE, 'utf8');
...
function _displayOptions(type, choices, options, callback) {
  ...
  if (type === TYPE_AUTO) {
    ...
    // so extracting all fct and putting them into
    // an array (for async.waterfall)
    res = _.map(choices[TYPE_AUTO], function (item) {
      return item.fct;
    });
