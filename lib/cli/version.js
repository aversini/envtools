...
EnvtoolsVersion.prototype._initialize = function () {
  ...
  if (needToCheck) {
    require('child_process')
      .spawn(process.execPath, [path.join(__dirname, '..', '..', 'bin', 'check.js')], {
        detached: true,
        stdio: 'ignore'
      }).unref();
  }
...
EnvtoolsVersion.prototype.setAutoCheck = function (flag) {
  ...
  config.setKey('envtoolsversion', _.extend(this._getConfiguration(), o), true, true);
};
...
EnvtoolsVersion.prototype.getAutoCheck = function () {
  ...
  if (cfg) {
    return (cfg[common.ENVTOOLS.CFG_AUTOCHECK]) ? cfg[common.ENVTOOLS.CFG_AUTOCHECK] : common.OFF;
  } else {
    return common.OFF;
  }
};
...
EnvtoolsVersion.prototype.printUpgradeIfAny = function (force) {
  var
    ...
    lightGrey = 245,
    rcInfo = this._getConfiguration() || {},
    LATEST_IS_INSTALLED = 'You are up-to-date!',
    NO_DATA_AVAILABLE = 'There is no data available at this time.\nPlease check again later...',
    AUTOCHECK_DISABLED = 'Automatically check for update is disabled.\nYou can enable this option by typing ' +
    log.strToColor(lightGrey, 'envtools config') +
    '\nand following the instructions at the prompt.';

  function printCustomMessage(msgs, boxColor) {
    log.echo();
    log.printMessagesInBox(msgs, (boxColor) ? boxColor : 'yellow');
    log.echo();
    return true;
  }
  ...
  if (force && rcInfo[common.ENVTOOLS.CFG_AUTOCHECK] !== common.ON) {
  ...
  if (rcInfo.latest) {
    if (require('semver').gt(rcInfo.latest, self.currentVersion)) {
      ...
      msg.push('Envtools ' +
        log.strToColor('green', rcInfo.latest) +
        ' is now available - you have ' +
        log.strToColor(lightGrey, self.currentVersion));
      msg.push('Type ' +
        log.strToColor(lightGrey, 'npm install -g envtools') +
        ' to upgrade.');
      msg.push('');
    } else {
      msg.push('');
      msg.push(LATEST_IS_INSTALLED);
      msg.push('');
    }
  }
  ...
  if (msg.length && (!rcInfo.alreadyDisplayed || force)) {
    ...
    return printCustomMessage(msg, 'green');
  }
};
