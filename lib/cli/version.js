...
EnvtoolsVersion.prototype._initialize = function () {
  ...
  if (needToCheck) {
    log.debug('==> spawn process to check latest version');
 ...
 * @param  {Boolean} force If treu, ignore the 'alreadyDisplayed' key
 */
EnvtoolsVersion.prototype.printUpgradeOption = function (force) {
  ...
  if ((rcInfo && !rcInfo.alreadyDisplayed && rcInfo.latest) ||
    ...
    if (require('semver').gt(rcInfo.latest, this.currentVersion)) {
      ...
      msg3 = this.i18n.t('version.changelogLink', {
        link: log.strToColor('cyan', 'http://goo.gl/135gWw')
      });
 ...
 */
EnvtoolsVersion.prototype.printCurrentVersion = function (done) {
  var
    questions,
    answers;
  ...
  questions = [{
    type: 'confirm',
    name: 'goodToGo',
    message: 'Do you want to see the changelog?',
    default: true
  }];
  require('inquirer').prompt(questions, function (answers) {
    if (!answers.goodToGo) {
      log.echo('Bye then...');
      log.echo();
      done(-1);
      ...
      utilities.openInBrowser({
        confirm: false,
        url: path.join(__dirname, '..', '..', 'envtools-history.html')
      });
      done();
    }
  });
