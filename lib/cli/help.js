...
/**
 * Print the long usage for envtools.
 *
...
EnvtoolsHelp.prototype.printLongUsage = function (intro) {
  ...
  var
    cmdtmp, cmdtmplen, cmdt, cmdl, cmdd, cmddlen, i, j,
    commandsList = _.keys(this.commands).sort(),
  ...
  for (i = 0; i < len; i += 1) {
    ...
    for (j = 1; j < descArrayLen; j += 1) {
      console.log(new Array(this.CMD_MAX_LEN + 1).join(' ') + descArray[j]);
    }
  }
            ...
            if (prop === 'options') {
              ...
              for (i = 0; i < len; i += 1) {
                if (!re.exec(_options[i].desc)) {
                  ...
                  res = utilities.wordWrap(_options[i].desc, process.stdout.columns - 12);
                  sublen = res.length;
              ...
              res = utilities.wordWrap(options[prop], process.stdout.columns - 12);
              len = res.length;
              for (i = 0; i < len; i += 1) {
