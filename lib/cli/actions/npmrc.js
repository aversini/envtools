...
function listProfiles(cfg) {
  var enabled,
    msgs = [],
    options = cfg || {},
    data = common.getExistingNpmrcProfiles();
  if (!_.isEmpty(data)) {
    if (data.enabled) {
...
module.exports = function (self, program) {
  var list = _.isBoolean(program.l) ? program.l : false,
    create = _.isString(program.c) ? program.c : null,
    profile = _.isString(program._[1]) ? program._[1] : null;
  ...
  if (list) {
    ...
    return common.switchToNpmrcProfile(profile, function () {
