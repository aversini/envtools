...
function getProxyStatus(callback) {
  var
    ...
    localProxyStatus = (process.env.PROXY_STATUS === common.ON),
    npmRegistry, yarnRegistry;

  fs.readFile(common.ENVTOOLS.PROXY_STATUS_FILE,
    function (err, data) {
      if (!err && data) {
        data = data.toString().replace(/\n$/, '');
        globalProxyStatus = (data === common.ON);
      }
      fs.readFile(common.ENVTOOLS.PROXY_FILE,
        function (err, data) {
          if (!err && data) {
            data = data.toString().replace(/\n$/, '');
            proxy = data;
          }
          fs.readFile(common.ENVTOOLS.SINOPIA_STATUS,
            function (err, data) {
              if (!err && data) {
                data = data.toString().replace(/\n$/, '');
                sinopiaStatus = (data === common.ON);
              }
              cmd.run('npm config get registry', {
                status: false
              }, function (err, stderr, stdout) {
                if (!err && stdout) {
                  npmRegistry = stdout.replace(/\n$/, '');
                }
                cmd.run('yarn config get registry', {
                  status: false
                }, function (err, stderr, stdout) {
                  if (!err && stdout) {
                  ...
                  return callback(null, {
                    proxy: proxy,
                    sinopiaStatus: sinopiaStatus,
                    localProxyStatus: localProxyStatus,
                    globalProxyStatus: globalProxyStatus,
...
function displayResults(data) {
  ...
  if (data.proxyData) {
    ...
    if (data.proxyData.localProxyStatus === data.proxyData.globalProxyStatus) {
      ...
      status = (data.proxyData.localProxyStatus) ? 'enabled, but only in current window' : 'disabled, but only in current window';
    }
  ...
  if (data.expiration) {
    ...
    log.printMessagesInBox(msg, common.LOG_COLORS.DEFAULT_BOX,
      'Last update: ' + moment(data.lastUpdate).format('hh:mm:ssa') +
