...
EnvtoolsCLI.prototype._runCommand = function (program, command, optimist) {
  ...
  case 'banner':
    ...
    msg.push('');
    log.printMessagesInBox(msg, null, 'v' + self.version.currentVersion);
    break;
    ...
    bootstrap.auto({
      ...
      if (notification && err) {
        notifier.notify({
          ...
          message: self.i18n.t('serve.errors.general'),
          sound: 'Blow',
          type: 'ERROR',
      ...
      if (err && err !== -1) {
        log.error(self.i18n.t('serve.errors.general'));
        log.info(self.i18n.t('serve.errors.info'));
        log.echo();
      }
    });
    break;
  ...
  case 'update':
  case 'upgrade':
    self.version.printCurrentVersion(function () {});
    break;
