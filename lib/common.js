...
function _unsetActiveNpmrcProfile() {
  var data = _getExistingNpmrcProfiles();
...
function _createNpmrcProfile(profileName, callback) {
  var profileDir = path.join(ENVTOOLS.NPMRC_PROFILES, profileName),
...
function _fuzzySearch(search, data, opts) {
  ...
  if (
    ...
    !_.isEmpty(result) && result.length &&
    !_.isEmpty(result[0].matches) && result[0].matches.length
  ) {
