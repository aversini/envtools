            ...
            if (e && typeof e == 'object') {
              for (c in e) {
                Object.prototype.hasOwnProperty.call(e, c) &&
                    ((d = h(e, c)), d !== undefined ? (e[c] = d) : delete e[c]);
              }
            }
            return f.call(a, b, e);
          })({'': g}, '')
...
function parseArgv(argv) {
  ...
  if (parsed.inPlace && parsed.inputFiles.length !== 1) {
    throw new Error(
      'must specify exactly one file with "-f FILE" to ' + 'use -I/--in-place'
    );
  }
  if (parsed.inPlace && parsed.args.length > 0) {
    throw new Error(
      'lookups cannot be specified with in-place editing ' +
...
function chunkEmitter(opts) {
    ...
    while (true) {
      if (s.slice(0, 5) === 'HTTP/') {
        ...
        if (index != -1) {
          ...
          const is100Continue = s.slice(0, 21) === 'HTTP/1.1 100 Continue';
  ...
  if (opts.inputFiles.length > 0) {
    ...
    function addEndListener(file) {
      file.on('end', function () {
        if (i < opts.inputFiles.length) {
          ...
          const nextFile = fs.createReadStream(next, {encoding: 'utf8'});
    ...
    const first = fs.createReadStream(opts.inputFiles[i++], {encoding: 'utf8'});
        ...
        switch (ch) {
          ...
          case ']':
            ...
            if (states[states.length - 1] === null) {
              const evaled = vm.runInNewContext(
                `(${bit.slice(1, -1)})`,
                {},
                '<lookup>'
              );
    ...
    case OM_COMPACT:
      ...
      if (datum === undefined) {
        ...
        datum.forEach(function (d) {
          bits.push('  ');
          bits.push(
            JSON.stringify(d, null, 0).replace(
              /* JSSTYLED */
              /,"(?![,:])/g,
              ', "'
            )
...
function main(argv) {
  ...
  const execVm = Boolean(process.env.JSON_EXEC && process.env.JSON_EXEC === 'vm');
  let i;
  const condFuncs = [];
  if (!execVm) {
  ...
  const lookups = lookupStrs.map(function (lookup) {
      ...
      while (true) {
        if (buffer.slice(0, 5) === 'HTTP/') {
          ...
          if (index != -1) {
            ...
            const is100Continue = buffer.slice(0, 21) === 'HTTP/1.1 100 Continue';
