...
if (filesList.length > 0) {
  filesList.forEach(function (file) {
    ...
    if (boring) {
      ...
      if (file && file.match && file.shortname) {
        name = common.LOG_COLORS.gray(name.replace(file.shortname,
          file.shortname.replace(file.match, common.LOG_COLORS.focusBg(file.match))));
      }
      log.rainbow('  %s%s%s%s%s%s',
  ...
  dirsList.forEach(function (dir) {
    ...
    if (boring) {
      ...
      if (dir && dir.match) {
        name = common.LOG_COLORS.blue(name.replace(dir.match, common.LOG_COLORS.focusBg(dir.match)));
      }
      log.rainbow('  %s%s%s%s%s',
