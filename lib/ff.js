...
function extractMode(mode) {
  var modeDec = parseInt(mode.toString(8), 10).toString().slice(-3),
    modeOwner = modeDec.charAt(0),
    modeGroup = modeDec.charAt(1),
    modeWorld = modeDec.charAt(2);
...
function convertSize(bytes) {
  ...
  str = parseInt(bytes, 10).toFixed(0) + sizes[posttxt];
...
function formatLongListings(stat, type) {
  var delim = (type === STR_TYPE_FILE) ? '-' : 'd';

  return {
    mode: delim + extractMode(stat.mode),
    owner: '  ' + posix.getpwnam(stat.uid).name,
    group: '  ' + posix.getgrnam(stat.gid).name,
