...
if (typeof program.pattern === 'string' || typeof program.pattern === 'number') {
...
function scanFiles(files, done) {
  var
    firstLog = true;
  each(files, function (strPath, cb) {
    var
      o,
      sOut,
      lines = [];
      ...
      if (lines.length) {
        ...
        if (program.grep) {
          lines.forEach(function (line) {
            ...
            line = common.LOG_COLORS.gray(line.replace(rePattern, common.LOG_COLORS.focusBg(res[0])));
