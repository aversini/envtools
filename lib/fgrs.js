...
/* Parsing options */
/*******************/
if (process.argv.length === 2 || program.args.length !== 1) {
  ...
  filesList.forEach(function (strPath) {
    var lines = [],
      readFileNode = function (err, str) {
        ...
        if (lines.length) {
          var o = (lines.length === 1) ? 'occurrence' : 'occurrences',
            sOut = '';
  ...
  if (filesList.length > 0) {
    ...
    scanFiles(function (err, data) {
      if (stats) {
        console.log('');
        if (data) {
          var o = (data.o === 1) ? 'occurrence found  : ' : 'occurrences found : ',
