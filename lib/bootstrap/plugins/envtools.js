...
module.exports = function (options, callback) {
  ...
  questions = [{
    ...
    when: function (res) {
      return _.indexOf(res.choices, envtoolsCfg.CFG_CUSTOM_PROMPT) >= 0;
    },
    ...
    choices: [{
      ...
      name: 'Prompt with Proxy, Sinopia, Node and Git information',
      short: 'Proxy, Sinopia and Node',
  ...
  inquirer.prompt(questions).then(function (answers) {
    ...
    function parseChoices(choices, val, label) {
      if (_.indexOf(choices, val) >= 0) {
        toggleOptions.msg.push(label + ': ' + log.strToColor('green', common.ON));
        toggleOptions[val] = common.ON;
      } else {
        toggleOptions.msg.push(label + ': ' + log.strToColor('yellow', common.OFF));
    ...
    parseChoices(choices.choices, envtoolsCfg.CFG_AUTOLOAD, envtoolsCfg.CFG_AUTOLOAD_LABEL);
    parseChoices(choices.choices, envtoolsCfg.CFG_CUSTOM_PROMPT, envtoolsCfg.CFG_CUSTOM_PROMPT_LABEL);
    parseChoices(choices.choices, envtoolsCfg.CFG_BANNER, envtoolsCfg.CFG_BANNER_LABEL);
    parseChoices(choices.choices, envtoolsCfg.CFG_AUTOCHECK, envtoolsCfg.CFG_AUTOCHECK_LABEL);

    options.toggleOptions = toggleOptions;
    options.toggleOptions.custom = (options.auto) ? defaults.custom : answers.custom;
    ...
    if (options.auto) {
      ...
      log.printMessagesInBox(options.toggleOptions.msg, common.LOG_COLORS.DEFAULT_BOX);
