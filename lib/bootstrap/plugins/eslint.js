var
  ...
  SRC_ESLINT_JSON = {
    ...
    ES6: require(path.join(common.ENVTOOLS.THIRDDIR,
      'eslint', 'rules', 'es6.js')),
    REACT: require(path.join(common.ENVTOOLS.THIRDDIR,
  ...
  ESLINT_CHOICES = [{
    ...
    value: 'ES6',
    short: 'es6',
    name: 'ECMAScript 6 ...... Specific rules for ES6'
  }, {
    checked: false,
    value: 'REACT',
    short: 'react',
    name: 'React/JSX ......... Specific rules for React/JSX'
  }];

module.exports = function (options, callback) {
  var
    DEFAULT_DEST_ESLINT_CFG = path.join(process.env.HOME, '.eslintrc.json');

  if (options.auto) {
    if (fs.existsSync(DEFAULT_DEST_ESLINT_CFG)) {
      backup(DEFAULT_DEST_ESLINT_CFG);
    }
    async.waterfall([
      function (done) {
        var
      ...
      function (done) {
        var
          data = _.defaultsDeep(
            SRC_ESLINT_JSON.STYLE,
            SRC_ESLINT_JSON.NODEJS,
            SRC_ESLINT_JSON.VARIABLES,
            SRC_ESLINT_JSON.BEST_PRACTICES,
            SRC_ESLINT_JSON.POSSIBLE_ERRORS,
            SRC_ESLINT_JSON.BASE
          );
        fs.writeFile(DEFAULT_DEST_ESLINT_CFG,
    ...
    async.waterfall([
      ...
      function (rules, done) {
        return done(null,
          _.defaultsDeep(
            (_.indexOf(rules, 'REACT') >= 0) ? SRC_ESLINT_JSON.REACT : '',
        ...
        var
          questions = [{
            ...
            choices: [{
              ...
              name: 'At the root of the HOME folder (not recommended)',
              short: 'home folder',
              value: HOME_FOLDER
            }]
          }];
