  ...
  async.waterfall(
    [
      ...
      function (goForIt, done) {
        var cmdline;
        if (goForIt && yarn) {
          cmdline = 'yarn config set registry http://registry.yarnpkg.com';
          cmd.run(cmdline, {
            status: !options.auto
          }, function () {
            done();
      ...
      function (packages, done) {
        if (packages && packages.length) {
          if (_.indexOf(packages, 'eslint') >= 0) {
            // adding extra eslint plugins automatically
            packages.push(ESLINT_PLUGINS);
          }
          return common.installNpmPackages(_.flatten(packages), done);
        } else {
          return done(common.USER_INTERRUPT);
        }
      }
