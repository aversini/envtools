var
  _ = require('lodash'),
  fs = require('fs-extra'),
  path = require('path'),
  async = require('async'),
  inquirer = require('inquirer'),
  cmd = require('fedtools-commands'),
  semver = require('semver'),
  utilities = require('fedtools-utilities'),
  log = require('fedtools-logs'),

  backup = require('../../backup'),
  common = require('../../common'),

  TYPE_SUDO = 100,
  TYPE_CHOWN = 200,

  NPM_CONFIG = path.join(process.env.HOME, '.npmrc'),
  NPM_PACKAGES = [{
    value: 'eslint',
    short: 'eslint',
    name: '[eslint]     A pattern checker/linter for JavaScript.'
  }, {
    value: 'fastlint',
    short: 'fastlint',
    name: '[fastlint]   A linter helper.'
  }, {
    value: 'grunt-cli',
    short: 'grunt-cli',
    name: '[grunt-cli]  The grunt command line interface.'
  }, {
    value: 'gulp-cli',
    short: 'gulp-cli',
    name: '[gulp-cli]   The gulp command line interface.'
  }, {
    value: 'selleck',
    short: 'selleck',
    name: '[selleck]    YUI documentation generator.'
  }, {
    value: 'shifter',
    short: 'shifter',
    name: '[shifter]    YUI code builder'
  }, {
    value: 'sinopia',
    short: 'sinopia',
    name: '[sinopia]    Private npm repository server.'
  }, {
    value: 'svgo',
    short: 'svgo',
    name: '[svgo]       Tool for optimizing SVG files.'
  }, {
    value: 'unicorn-tool',
    short: 'unicorn',
    name: '[unicorn]    Framework code builder.'
  }, {
    value: 'webpack',
    short: 'webpack',
    name: '[webpack]   Another module bundler.'
  }, {
    value: 'yogi',
    short: 'yogi',
    name: '[yogi]       YUI gallery code builder.'
  }, {
    value: 'yuidocjs',
    short: 'yuidocjs',
    name: '[yuidocjs]   YUI JavaScript Documentation engine.'
  }];


function _displayNodeWarningWithProxy(done) {
  var
    version = process.version,
    isMac = process.platform === 'darwin',
    isProxy = fs.existsSync(common.ENVTOOLS.PROXY_FILE),
    isNode = semver.gte(process.version, '6.0.0'),
    isSinopia = (process.env.SINOPIA_STATUS === common.ON ||
      process.env.SINOPIA_STATUS === common.OFF),
    msg = [];

  if (isMac && isProxy && isNode && !isSinopia) {
    msg.push(log.strToColor('red', '                         W A R N I N G\n'));
    msg.push('It seems that you have a proxy and are using Node ' + version + '...');
    msg.push('There is a known issue that will render your ' + log.strToColor('cyan', 'npm install') + ' commands');
    msg.push('extremelly slow... It is recommended that you ' + log.strToColor('red', 'do not install') + ' any');
    msg.push('node packages before installing and configuring Sinopia.');
    msg.push('');
    msg.push('Please see the corresponding help page by typing ' + log.strToColor('cyan', 'h') + ' + ENTER at the');
    msg.push('command line prompt. Thanks!');
    log.printMessagesInBox(msg);
    return done(1);
  }
  return done();
}

function _installPackagesWithAction(action, packages, destination, done) {
  var
    cmdline;

  if (action === TYPE_SUDO) {
    cmdline = 'npm install -g ' + packages.join(' ');
    cmd.sudo(cmdline, {}, function (err, stderr) {
      log.debug(stderr);
      if (err) {
        log.error('Something went wrong or you did not grant admin access...');
      }
      return done(err);
    });
  } else if (action === TYPE_CHOWN) {
    common.chownFolder(destination, {
      debug: false
    }, function (err) {
      log.debug(err);
      if (err) {
        log.error('Something went wrong or you did not grant admin access...');
      }
      return done(err);
    });
  } else {
    return done(common.USER_INTERRUPT);
  }
}

module.exports = function (options, callback) {
  backup(NPM_CONFIG);
  async.waterfall(
    [
      function (done) {
        var
          questions = {
            type: 'confirm',
            name: 'goForIt',
            message: 'About to update npm configuration, continue?',
            default: true
          };
        inquirer.prompt(questions).then(function (answers) {
          options.actionsPending++;
          if (answers.goForIt) {
            options.actionsDone++;
            return done(null, 1);
          } else {
            return done(null, 0);
          }
        });
      },
      function (goForIt, done) {
        var cmdline;
        if (goForIt) {
          if (process.env.SINOPIA_STATUS && process.env.SINOPIA_STATUS === common.ON) {
            cmdline = 'npm config set registry http://localhost:4873/';
          } else {
            cmdline = 'npm config set registry=http://registry.npmjs.org/';
          }
          cmd.run(cmdline, {
            status: (options.auto) ? false : true
          }, function () {
            done();
          });
        } else {
          return done();
        }
      },
      function (done) {
        var
          questions = {
            type: 'confirm',
            name: 'goForIt',
            message: 'Continue?',
            default: false
          };
        _displayNodeWarningWithProxy(function (err) {
          if (!options.auto && err) {
            // need to ask a simple 'continue?' in manual mode
            inquirer.prompt(questions).then(function (answers) {
              if (answers.goForIt) {
                return done();
              } else {
                return done(common.USER_INTERRUPT);
              }
            });
          } else {
            return done();
          }
        });
      },
      function (done) {
        var
          questions = {
            type: 'confirm',
            name: 'goForIt',
            message: 'About to install core node packages, continue?',
            default: true
          };
        if (!options.auto) {
          // no need to ask the question in manual mode
          return done(null, 1);
        } else {
          inquirer.prompt(questions).then(function (answers) {
            options.actionsPending++;
            if (answers.goForIt) {
              options.actionsDone++;
              return done(null, 1);
            } else {
              return done(null, 0);
            }
          });
        }
      },
      function (goForIt, done) {
        if (goForIt) {
          // in manual mode, let's ask the user to choose
          // which package they want to install...
          if (!options.auto) {
            inquirer.prompt([{
              type: 'checkbox',
              message: 'Select one or more node packages you want to install',
              name: 'npms',
              choices: NPM_PACKAGES,
              pageSize: NPM_PACKAGES.length + 1,
              validate: function (val) {
                if (!val.length) {
                  return 'Press <space> to select one or more packages, or <ctrl-c> to quit...';
                }
                return true;
              }
            }]).then(function (answers) {
              return done(null, answers.npms);
            });
          } else {
            // in auto mode, install all packages
            return done(null, NPM_PACKAGES);
          }
        } else {
          return done(common.USER_INTERRUPT);
        }
      },
      function (packages, done) {
        if (packages && packages.length) {
          return done(null, packages);
        } else {
          return done(common.USER_INTERRUPT);
        }
      },
      function (packages, done) {
        var
          questions = {
            type: 'list',
            name: 'action',
            message: 'Please choose one of the following options',
            choices: [{
              value: TYPE_CHOWN,
              short: 'chmod',
              name: 'Change ownership of the destination folders to YOUR user (preferable)'
            }, {
              value: TYPE_SUDO,
              short: 'sudo',
              name: 'Install packages with sudo to overcome permission issue (not advised)'
            }]
          },
          rootNodeModules,
          destNodeModules = [];

        rootNodeModules = utilities.getGlobalNodeModulesPath();
        destNodeModules.push(rootNodeModules);
        _.each(packages, function (p) {
          destNodeModules.push(path.join(rootNodeModules, p));
        });
        utilities.isFolderWritable(destNodeModules, function (err) {
          if (err) {
            log.warning('Destination folders are not writable...');
            inquirer.prompt(questions).then(function (answers) {
              return done(null, rootNodeModules, packages, answers.action);
            });
          } else {
            return done(null, null, packages, false);
          }
        });
      },
      function (destination, packages, action, done) {
        var
          actions = _.map(packages, function (pkg) {
            return function (iamdone) {
              var
                res,
                name = (pkg.value) ? pkg.value : pkg;

              res = cmd.run('npm install -g ' + name, {
                status: true
              });
              if (res.code !== 0 && res.stderr) {
                log.rainbow(res.stderr);
              }
              iamdone();
            };
          });

        if (action === false) {
          async.waterfall(actions, done);
        } else {
          _installPackagesWithAction(action, packages, destination, done);
        }
      }
    ],
    function (err) {
      if (options.auto && err === common.USER_INTERRUPT) {
        err = null;
      }
      if (!err && !options.auto) {
        err = common.USER_IGNORE;
      }
      callback(err, options);
    });
};
