...
function txtYellow {
  txtColor "$COLOR_YELLOW" "$@"
}
function txtMagenta {
  txtColor "$COLOR_MAGENTA" "$@"
}
function txtCyan {
  txtColor "$COLOR_CYAN" "$@"
}
function txtDefault {
  txtColor "$COLOR_DEFAULT" "$@"
}

function txtBoldRed {
  txtColor "$COLOR_B_RED" "$@"
}
function txtBoldBlue {
  txtColor "$COLOR_B_BLUE" "$@"
}
function txtBoldGreen {
  txtColor "$COLOR_B_GREEN" "$@"
}
function txtBoldYellow {
  txtColor "$COLOR_B_YELLOW" "$@"
}
function txtBoldMagenta {
  txtColor "$COLOR_B_MAGENTA" "$@"
}
function txtBoldCyan {
  txtColor "$COLOR_B_CYAN" "$@"
}

function txtColor {
  if shouldLog; then
    printf "$1%s$COLOR_DEFAULT" "$2"
    if [ $# -eq 3 -a "$3" == "nl" ]; then
      echo
    fi
  fi
